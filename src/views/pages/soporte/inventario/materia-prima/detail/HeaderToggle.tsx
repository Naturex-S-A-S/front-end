...
import { toggleFeedstockActive } from "@/api/feedstock/actions";
...
export default function HeaderToggle({ id, active }: Props) {
  ...
  const canUpdate = ability.can(ABILITY_ACTIONS.UPDATE as any, ABILITY_SUBJECT.FEEDSTOCK, ABILITY_FIELDS.LISTADO);
  ...
  const handleToggle = async () => {
    ...
    const result = await toggleFeedstockActive(id, nextActive);

    if (!result.success) {
      toast.error(result.error || "Error al actualizar la materia prima");
    }
  };
